...
######################################################################
setwd(
  "/nfs/research/icortes/DATA/SAVANA_manuscript/revision/benchmarking_code/normal_replicates"
)
#--------------------------------------------------------------------------------------------------------------------------
...
#--------------------------------------------------------------------------------------------------------------------------
# Normal split replicate analysis
#--------------------------------------------------------------------------------------------------------------------------
samples = unique(sample_info$donor_id)
length(samples)
# only keep samples that were included in normal vs normal split
normal_samples = read.table(
  "data/normal_samples.tsv",
  header = T,
  sep = "\t"
)
normal_samples <- as.list(normal_samples$donor)
...
for (sample in normal_samples) {
  for (caller in callers) {
    ...
    if (caller == "nanomonsv-0.5.0") {
      ...
      if(length(file_nows) != 1){ print("Incorrect number of files matched!"); print(length(file_nows)); print(file_nows)}
      for (file_now in file_nows) {
        ...
        if (!is.na(file_now) && file.exists(file_now)) {
          ...
          file_now = list.files(path = path_now,
                                pattern = "normal_0.nanomonsv.result.txt",
                                full.names = T)
          if (!is.na(file_now)) {
            now = read.table(
              file_now,
          ...
          sbnd_file_now = list.files(path = path_now,
                                     pattern = "normal_0.nanomonsv.sbnd.result.txt",
                                     full.names = T)
          now = NA
          sbnd_variants = NA
      ...
      if(!is.null(nanomon_sv_sample_all)){
        ...
        if (!is.na(sbnd_variants)){
          ...
          all_nanomon_variants = non_sbnd_variants
        }
...
saveRDS(all, file = "rds/SVs_normal_splits_raw.rds")
#------------------------------------------------------------------------------------
# add annotations
#------------------------------------------------------------------------------------
...
#----
saveRDS(all, file = "rds/SVs_normal_splits_bps.rds")
