######################################################################
## Script to load replicate SV calls
## Adapted from ICC by HE
## 20/10/2024
######################################################################
setwd(
  "/nfs/research/icortes/DATA/SAVANA_manuscript/revision/benchmarking_code/tumour_replicates"
)
#--------------------------------------------------------------------------------------------------------------------------
  ...
  for (caller in callers) {
    ...
    if (caller == "nanomonsv-0.5.0") {
      path_now = paste0(
        ...
        caller,
        "-sbnd/results/split_tumour_bams/comparisons/",
        sample
      )
      file_nows = list.files(path = path_now,
                             pattern = "Genes.vcf",
                             full.names = T)
      nanomon_sv_sample_all = c()
      if(length(file_nows) != 2){ print("Incorrect number of files matched!"); print(length(file_nows)); print(file_nows)}
      for (file_now in file_nows) {
        print(file_now)
        if (!is.na(file_now) && file.exists(file_now)) {
          # is it from the first or second split?
          split_x <- sub(".*\\.(split-[0-9]+)_.*", "\\1", file_now)
          vcf <- readVcf(file_now)
          gr <-
            breakpointRanges(vcf, info_columns = annotation_info_fields)
          bedpe <- data.frame(
            chrom1 = seqnames(gr),
            start1 = start(gr) - 1,
            end1 = end(gr),
            ...
            LABEL = gr$LABEL,
            LABEL_VARIANT_ID = gr$LABEL_VARIANT_ID,
            DISTANCE_TO_MATCH = gr$DISTANCE_TO_MATCH,
            file = split_x
          )
          ...
          path_now = paste0(
            "/nfs/research/icortes/DATA/SAVANA_manuscript/tool_benchmark/",
            caller,
            "-sbnd/results/split_tumour_bams/",
            sample,
            "/",
            split_x,
            "/tumour"
          ...
          if (!is.na(sbnd_file_now)) {
            ...
            now$CENTROMERE = NA
            now$GENE = NA
            now$LABEL_VARIANT_ID = NA
            now$DISTANCE_TO_MATCH = NA 
            now$SUPPORT = now$Supporting_Read_Num_Tumor
            now$AF = now$Supporting_Read_Num_Tumor / now$Checked_Read_Num_Tumor
            # add temp validation label
            now$LABEL = "NOT_IN_COMPARISON"
            now$end1 = now$start1
            ...
            # add bedpe to all svs for this sample
            nanomon_sv_sample_all = rbind(nanomon_sv_sample_all, bedpe)
          }
        }
        else{
          print(paste("FAILED:", cohort, sample, caller))
        }
      }
      if(!is.null(nanomon_sv_sample_all)){
        # here we compare the single breakends in nanomon_sv_sample_all
        # Example data
        #nanomon_sv_sample_all <- data.frame(
        #  name = c("variant1", "variant2", "variant3"),
        #  start1 = c(10000, 10010, 10020),
        #  chrom1 = c("chr1", "chr1", "chr1"),
        #  file = c("split-0", "split-1", "split-1"),
        #  svtype = c("SBND", "SBND", "SBND"),
        #  LABEL = c("NOT", "NOT", "NOT"),
        #  DISTANCE_TO_MATCH = c(NA, NA, NA),
        #  LABEL_VARIANT_ID = c(NA, NA, NA)
        #)
        nanomon_sv_sample_all <- as.data.frame(nanomon_sv_sample_all)
        sbnd_variants <- nanomon_sv_sample_all[nanomon_sv_sample_all$svtype == "SBND", ]
        
        # Track already matched variants
        used_variants <- vector("logical", length = nrow(sbnd_variants))
        names(used_variants) <- sbnd_variants$name
        
        # Iterate through the rows of the dataframe
        for (i in 1:nrow(sbnd_variants)) {
          
          # Skip already matched variants
          if (used_variants[i]) next
          
          # Get the current row
          current_row <- sbnd_variants[i, ]
          
          # Skip if not single breakend
           
          if (current_row$svtype != "SBND") next         
          
          
          # Initialize variables to track the closest match
          closest_match_idx <- NA
          closest_distance <- Inf
          
          # Loop through all rows to find potential matches (excluding current row)
          for (j in 1:nrow(sbnd_variants)) {
            
            # Skip comparing the current row to itself or an already matched variant
            if (i == j || used_variants[j]) next
            
            other_row <- sbnd_variants[j, ]
            
            # Check if this row is a valid match
            if (other_row$svtype == "SBND" && 
                current_row$chrom1 == other_row$chrom1 && 
                abs(current_row$start1 - other_row$start1) <= 100 && 
                current_row$file != other_row$file) {
              
              # Calculate the distance between the current row and the other row
              distance <- abs(current_row$start1 - other_row$start1)
          ...
          # If a match was found, update the corresponding fields
          if (!is.na(closest_match_idx)) {
            sbnd_variants$LABEL_VARIANT_ID[i] <- sbnd_variants$name[closest_match_idx]
            sbnd_variants$DISTANCE_TO_MATCH[i] <- closest_distance
            sbnd_variants$LABEL[i] <- "SOMATIC"
            # also do the same for the matched variant
            sbnd_variants$LABEL_VARIANT_ID[closest_match_idx] <- sbnd_variants$name[i]
            sbnd_variants$DISTANCE_TO_MATCH[closest_match_idx] <- closest_distance
            sbnd_variants$LABEL[closest_match_idx] <- "SOMATIC"
            # Mark this variant as matched
            used_variants[i] <- TRUE
            used_variants[closest_match_idx] <- TRUE
          }
        }
        sbnd_variants <- as.data.frame(sbnd_variants)
        nanomon_sv_sample_all <- as.data.frame(nanomon_sv_sample_all)
        non_sbnd_variants <- nanomon_sv_sample_all[nanomon_sv_sample_all$svtype != "SBND", ]
        
        all_nanomon_variants = rbind(sbnd_variants,non_sbnd_variants)
        all = rbind(all, all_nanomon_variants)
      }
    }
    if (caller == "sniffles-2.2.0") {
      path_now = paste0(
        "/nfs/research/icortes/DATA/SAVANA_manuscript/tool_benchmark/",
        caller,
        "/results/split_tumour_bams/comparisons/",
        sample
      )
      file_nows = list.files(path = path_now,
                             pattern = "Genes.vcf",
    ...
    if (caller == "severus-1.0") {
      path_now = paste0(
        "/nfs/research/icortes/DATA/SAVANA_manuscript/tool_benchmark/",
        caller,
        "/results/split_tumour_bams/comparisons_corrected/",
        sample
      )
      file_nows = list.files(path = path_now,
                             pattern = "Genes.vcf",
      ...
      for (file_now in file_nows) {
        ...
        if (!is.na(file_now)) {
          ...
          bedpe = bedpe[which(bedpe$svtype != "BND" | !grepl("svrecord", bedpe$name)),]
        ...
        if (!is.na(file_now)) {
          ...
          if (nrow(data.frame(begr)) > 0) {
            ...
            bedpe_sbnd <- data.frame(
              ...
              LABEL_VARIANT_ID = begr$LABEL_VARIANT_ID,
              DISTANCE_TO_MATCH = begr$DISTANCE_TO_MATCH
            )
...
table(all$caller)
table(all$caller, all$LABEL)
table(all$caller, all$file)

saveRDS(all, file = "rds/SVs_tumour_splits_raw.rds")
#------------------------------------------------------------------------------------
# add annotations
#------------------------------------------------------------------------------------
...
# don't need bp2 of insertions
# second one has "_bp2" so we remove it and then the "_bp1" modifier from remaining
all <- all[!(all$svtype == "INS" & grepl("_bp2$", all$name)),]

# swap the strands (when loaded, they're reversed for _2 edge of BNDS - both standard and bp2)
all <- all %>%
  dplyr::mutate(
...
idx = which(all$chrom1 == all$chrom2 & all$svtype != "INS" & all$start2 != '.')
all$abs_svLen[idx] = abs(as.numeric(all$start1[idx]) -  as.numeric(all$start2[idx]))
...
table(all$SV_type, all$caller, all$LABEL)
table(all$SV_type, all$caller)

#----
saveRDS(all, file = "rds/SVs_tumour_splits_bps.rds")
